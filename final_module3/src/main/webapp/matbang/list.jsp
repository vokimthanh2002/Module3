...
<h1 style="text-align: center">Danh sách mat bang</h1>
<div class="row flex-column align-items-end mr-5" >
    <nav class="navbar navbar-light bg-light justify-content-end">
        ...
        <button class="btn btn-success" type="submit" onclick="window.location.href='/matbang?action=create'"> Them moi mat bang </button> &emsp;
        <form  class="form-inline" method="get" >
            <input class="form-control mr-sm-2" type="search"  placeholder="Gia cho thue" aria-label="Search" name="inputSearch">
...
<table class="table table-sm table-dark" class="table" id="tableStudent">
    ...
    <c:forEach items="${matBangList}" var="matbang">
        <tr>
            <td scope="row"><a href="/matbang?action=view&id=${matbang.getMaMatBang()}">${matbang.getMaMatBang()}</a></td>
            <th scope="row">${matbang.getDienTich()}</th>
            <th scope="row">${matbang.getTrangThai()}</th>
            <th scope="row">${matbang.getTang()}</th>
            <th scope="row">${matbang.getLoaiMatBang()}</th>
            <th scope="row">${matbang.getGiaTien()}</th>
            <th scope="row">${matbang.getNgayBatDau()}</th>
            <th scope="row">${matbang.getNgayKetthuc()}</th>
            <th scope="row">
                <button type="submit" class="btn btn-info" onclick="window.location.href='/matbang?action=edit&id=${matbang.getMaMatBang()}'"> Chỉnh Sửa </button>
                <button type="submit" class="btn btn-danger"  onclick="window.location.href='/matbang?action=delete&id=${matbang.getMaMatBang()}'"> Xóa </button>
            </th>
        </tr>
    </c:forEach>
    </tbody>
